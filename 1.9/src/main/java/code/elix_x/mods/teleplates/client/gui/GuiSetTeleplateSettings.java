...
public class GuiSetTeleplateSettings extends GuiScreen {
	...
	public void initGui(){
		...
		textFieldName = new GuiTextField(0, fontRendererObj, guiLeft, guiTop, 128, 20);
		textFieldName.setText(name);
		this.buttonList.add(buttonModePublic = new GuiButton(1, guiLeft, guiTop + 28, 40, 20, I18n.translateToLocal("teleplates.gui.settings.mode.public")){
		...
		this.buttonList.add(buttonModeProtected = new GuiButton(2, guiLeft + 44, guiTop + 28, 40, 20, I18n.translateToLocal("teleplates.gui.settings.mode.protected")){
		...
		this.buttonList.add(buttonModePrivate = new GuiButton(3, guiLeft + 88, guiTop + 28, 40, 20, I18n.translateToLocal("teleplates.gui.settings.mode.private")){
		...
		this.buttonList.add(buttonUsingList = new GuiButton(4, guiLeft, guiTop + 56, 128, 20, I18n.translateToLocal(usingList ? "teleplates.gui.settings.mode.protected.list" : "teleplates.gui.settings.mode.protected.password")));
		buttonUsingList.visible = mode == EnumTeleplateMode.PROTECTED;

		textFieldPassword = new GuiTextField(0, fontRendererObj, guiLeft, guiTop + 84, 128, 20);
		if(password != null) textFieldPassword.setText(password);

		this.buttonList.add(buttonWhitelist = new GuiButton(5, guiLeft, guiTop + 84, 128, 20, I18n.translateToLocal(whitelist ? "teleplates.gui.settings.mode.protected.list.whitelist" : "teleplates.gui.settings.mode.protected.list.blacklist")));
		...
		textFieldAddPlayer = new GuiTextField(0, fontRendererObj, guiLeft, guiTop + 184, 62, 20);
		this.buttonList.add(buttonAddPlayer = new GuiButton(6, guiLeft + 66, guiTop + 184, 62, 20, I18n.translateToLocal("teleplates.gui.settings.mode.protected.list.add")));
		buttonAddPlayer.visible = mode == EnumTeleplateMode.PROTECTED && usingList;

		llist = list != null ? list.toArray(new String[0]) : new String[0];

		this.buttonList.add(new GuiButton(0, guiLeft, guiTop + ySize - 20, 128, 20, I18n.translateToLocal("teleplates.gui.settings.confirm")));
	}
	...
	public class GuiPlayerList extends GuiListExtended {
		...
		public IGuiListEntry getListEntry(int i){
			return new IGuiListEntry(){
				...
				@Override
				public void drawEntry(int i, int x, int y, int relX, int relY, int p_148279_7_, int p_148279_8_, boolean p_148279_9_){
