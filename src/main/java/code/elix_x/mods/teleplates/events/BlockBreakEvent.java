	...
	public void breakBlock(BreakEvent event){
		if(event.block == TeleplatesBase.teleplate){
			if(!event.world.isRemote){
				if(ConfigurationManager.permissionsSystemActive() && !TeleplatesManager.isAdmin(((TileEntityTeleplate) event.world.getTileEntity(event.x, event.y, event.z)).getTeleplateId(), event.getPlayer())){
